// Code generated by "stringer -linecomment -output ast_string.go -type ConstNode,BinaryOperator,UnaryOperator,MethodNode"; DO NOT EDIT.
...
const _ConstNode_name = "$@last[*]*truefalsenull"

var _ConstNode_index = [...]uint8{0, 1, 2, 6, 9, 10, 14, 19, 23}

func (i ConstNode) String() string {
	if i < 0 || i >= ConstNode(len(_ConstNode_index)-1) {
		return "ConstNode(" + strconv.FormatInt(int64(i), 10) + ")"
	}
	return _ConstNode_name[_ConstNode_index[i]:_ConstNode_index[i+1]]
}
func _() {
	// An "invalid array index" compiler error signifies that the constant values have changed.
	// Re-run the stringer command to generate them again.
...
const _MethodNode_name = ".abs().size().type().floor().ceiling().double().keyvalue().bigint().boolean().date().integer().number().string()"

var _MethodNode_index = [...]uint8{0, 6, 13, 20, 28, 38, 47, 58, 67, 77, 84, 94, 103, 112}

func (i MethodNode) String() string {
	if i < 0 || i >= MethodNode(len(_MethodNode_index)-1) {
		return "MethodNode(" + strconv.FormatInt(int64(i), 10) + ")"
	}
	return _MethodNode_name[_MethodNode_index[i]:_MethodNode_index[i+1]]
}
